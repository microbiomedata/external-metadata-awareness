...
//   mongosh ... --eval "var __targetCollection='global_nmdc_slots_test';" --file load_global_nmdc_slots.js
...
const nmdcGitRepo = 'https://github.com/microbiomedata/nmdc-schema.git';
const nmdcRef = 'refs/heads/main';
const localSchemaFile = 'local/nmdc_materialized_patterns.yaml';
const tempJsonFile = '/tmp/global_nmdc_slots.json';

const targetCollection = (typeof __targetCollection !== 'undefined') ? __targetCollection : 'global_nmdc_slots';
...
try {
    ...
    // 0.24s cold — 10s is ~40x the cold-case observation.
    const lsRemoteOut = execSync(
        `git ls-remote "${nmdcGitRepo}" "${nmdcRef}"`,
        {timeout: 10000, stdio: ['ignore', 'pipe', 'pipe']}
    ).toString().trim();
    upstreamSha = lsRemoteOut.split(/\s+/)[0] || null;
    print(`[${new Date().toISOString()}] Upstream ${nmdcRef} SHA: ${upstreamSha}`);
} catch(e) {
...
try {
    ...
    try {
        coll.createIndex({slot_uri: 1}, {background: true});
    } catch(e) {
    ...
    print(`  Slots with ranges: ${coll.countDocuments({range: {$exists: true, $ne: null}})}`);
    print(`  Slots with slot_uri: ${coll.countDocuments({slot_uri: {$exists: true, $ne: null}})}`);
    print(`  Slots with annotations: ${coll.countDocuments({annotations: {$exists: true}})}`);
    print(`  Slots with min/max values: ${coll.countDocuments({$or: [{minimum_value: {$exists: true}}, {maximum_value: {$exists: true}}]})}`);
