...
# =============================================================================
def fetch_nmdc_submissions():
    """
    Fetch NMDC submissions from the API and insert them into the local MongoDB
    collection 'nmdc_submissions' in the 'misc_metadata' database.
    """
    # Load environment variables from .env file
    env_path = "../../local/.env"  # Adjust path as needed
    env_vars = dotenv_values(env_path)
    refresh_token = env_vars.get('NMDC_DATA_SUBMISSION_REFRESH_TOKEN')
    if not refresh_token:
        print("NMDC_DATA_SUBMISSION_REFRESH_TOKEN not found in env.")
        return
    ...
    # Optionally clear existing submissions:
...
# =============================================================================
# SETUP: SCHEMA USAGE & ONTOLOGY VARIABLES
# =============================================================================
nmdc_schema_url = (
...
# Define non-environmental label-check keys (used later)
to_label_check = [
...
def create_biosample_rows():
    ...
    if biosample_rows:
        ...
        print(f"Inserted {len(result.inserted_ids)} documents into 'submission_biosample_rows' collection.")
...
def run_aggregation_pipeline():
    ...
    print("Aggregation pipeline executed; output written to collection 'submission_biosample_slot_counts'.")
...
# =============================================================================
def main():
    # Fetch NMDC submissions from the API and insert into MongoDB.
    fetch_nmdc_submissions()
    # Process the submissions to create flattened biosamples and export TSV.
    process_submissions()
    # Create biosample rows as per the notebook transformation.
    create_biosample_rows()
    # Run the aggregation pipeline on the biosample rows.
