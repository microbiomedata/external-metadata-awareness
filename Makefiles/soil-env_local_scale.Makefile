...
# doesn't contain current PVs anymore
...
local/EnvLocalScaleSoilEnum-pvs-keys-parsed-unique.csv
	$(RUN) python external_metadata_awareness/extract_value_set_evidence.py \
		--config $< \
		--downsample-uncounted \
		--output-file $@

local/soil-env-local-scale-evidence-distribution.png: local/soil-env-local-scale-evidence-table.tsv
...
aggressive-soil-env-local-scale-cleanup:
	...
	rm -rf local/EnvLocalScaleSoilEnum*
	...
	rm -rf local/soil-env-local-scale-evidence-distribution.png
