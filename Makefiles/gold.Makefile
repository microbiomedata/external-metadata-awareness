...
local/goldterms-env_broad_scale-of-environmental-terrestrial-soil.tsv: local/envo_goldterms.db # counts by path,  not by bold biosamples
	$(RUN) runoak --input $< query --output $@.bak --query "SELECT s.subject, SUBSTR(s.subject, INSTR(s.subject, ':') + 1) AS gold_path_id_int, s.object as inferred_env_broad_scale, count(1) as path_count FROM entailed_edge ee JOIN statements s ON ee.subject = s.subject WHERE ee.predicate = 'rdfs:subClassOf' AND ee.object = 'GOLDTERMS:4212' AND s.predicate = 'mixs:env_broad' group by s.object order by count(1) desc"
	cut -f1,2,3,5,6 $@.bak > $@
	rm -rf $@.bak
...
local/goldterms-env_medium-of-environmental-terrestrial-soil.tsv: local/envo_goldterms.db # counts by path,  not by bold biosamples
	$(RUN) runoak --input $< query --output $@.bak --query "SELECT s.subject, SUBSTR(s.subject, INSTR(s.subject, ':') + 1) AS gold_path_id_int, s.object as inferred_env_medium, count(1) as path_count FROM entailed_edge ee JOIN statements s ON ee.subject = s.subject WHERE ee.predicate = 'rdfs:subClassOf' AND ee.object = 'GOLDTERMS:4212' AND s.predicate = 'mixs:env_medium' group by s.object order by count(1) desc"
	cut -f1,2,3,5,6 $@.bak > $@
	rm -rf $@.bak
